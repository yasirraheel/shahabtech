            ...
            <div class="row">
                <div class="header-wrapper">
                    ...
                    <div class="logo-wrapper">
                        <a href="{{route('home')}}" class="normal-logo"> <img src="{{ siteLogo() }}" alt="{{ gs('site_name')}}">
                        </a>

                        <a href="{{route('home')}}" class="dark-logo hidden"  > <img src="{{ siteLogo('dark') }}" alt="{{ gs('site_name')}}">
                        </a>
                    </div>
    ...
    <div class="top-close d-flex align-items-center justify-content-between">
        <div class="header-wrapper">
            <div class="logo-wrapper mb-1">
                <a href="{{route('home')}}" class="normal-logo"> <img src="{{ siteLogo() }}" alt="{{ gs('site_name') }}"></a>
                <a href="{{route('home')}}" class="dark-logo hidden" > <img src="{{ siteLogo('dark') }}" alt="{{ gs('site_name') }}"></a>
            </div>
        </div>
            ...
            @foreach($pages as $k => $data)
                @if($data->link_type == 2)
                    <li class="sidebar-menu-list__item">
                        <a href="{{ $data->url ?? '' }}" target="_blank" class="sidebar-menu-list__link">{{__($data->title)}}</a>
                    </li>
                @else
                    <li class="sidebar-menu-list__item  {{ route('pages',[$data->url]) == url()->current() ? 'active' : null }}">
                        <a href="{{route('pages',[$data->url])}}" class="sidebar-menu-list__link">{{__($data->title)}}</a>
                    </li>
                @endif
            @endforeach
        @endif
