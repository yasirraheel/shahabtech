...
class PaymentController extends Controller
{
     ...
     public function servicePlace(Request $request){
        ...
        if($request->gateway == 'balance'){
            ...
            $transaction->trx           =   $trx;
            $transaction->remark        =   'order_place';
            $transaction->save();
    ...
    public function depositInsert(Request $request)
    {
        ...
        if($request->gateway == 'balance'){
            ...
            if($plan->month !=null){
                ...
                $planSubscribe->starts_at = now();
                $planSubscribe->ends_at = now()->addDays((int) $plan->month);
                $planSubscribe->save();
            }else{
                $planSubscribe=new Subscription();
                $planSubscribe->user_id= $user->id;
                ...
                $planSubscribe->amount= $plan->price;
                $planSubscribe->starts_at = now();
                $planSubscribe->ends_at = now()->addDays((int) $plan->year);
                $planSubscribe->save();
            }
        ...
        if ($deposit->status == Status::PAYMENT_PENDING || $deposit->status == Status::PAYMENT_INITIATE) {
            ...
            if(isset($deposit->plan_id)){
                ...
                if($plan->month !=null){
                    ...
                    $subscribe->starts_at = now();
                    $subscribe->ends_at = now()->addDays((int) $plan->month);
                    $subscribe->save();
                }else{
                    $subscribe=new Subscription();
                    $subscribe->user_id= $user->id;
                    ...
                    $subscribe->amount= $plan->price;
                    $subscribe->starts_at =  now();
                    $subscribe->ends_at =  now()->addDays((int) $plan->year);
                    $subscribe->save();
                }
            ...
            if(isset($deposit->order_id)){
                ...
                $adminNotification->click_url = urlPath('admin.deposit.log');
                $adminNotification->save();
